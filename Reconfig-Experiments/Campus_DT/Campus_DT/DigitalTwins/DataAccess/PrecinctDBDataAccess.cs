...
namespace DataAccess
{
    public class PrecinctDBDataAccess
    {
        ...
        public async Task<List<EnergyMeterModel>> GetEnergyReading(string precinct, string timestamp, int meterid, string type)
        {
            var energymeterCollection = ConnectToMongo<EnergyMeterModel>(EnergyCollection);
            if (type == null)
            {
                var results = await energymeterCollection.FindAsync(c => (c.EnergyMeter_name == precinct && c.Timestamp == timestamp && c.Meter_ID == meterid && (c.Type != "Day Max" || c.Type != "Month Max" || c.Type != "Year Max")));
                return results.ToList();
            }
            else
            {
                var results = await energymeterCollection.FindAsync(c => (c.EnergyMeter_name == precinct && c.Timestamp == timestamp && c.Meter_ID == meterid && c.Type == type));
                return results.ToList();
            }
        }
